...
const CHECK_TIMEOUT_MS = Math.max(15000, Number(process.env.CHECK_TIMEOUT_MS || 60000));
...
app.post('/site/:siteId/api/jobs', auth, siteAuth, (req, res) => {
  ...
  const job = { id, siteId: req.site.id, siteName: req.site.name, ids, period, status: 'queued', createdAt: nowWib(), createdBy: req.session?.user || req.get('X-CRM-User') || 'unknown', createdByRole: req.session?.crmRole || req.get('X-CRM-Role') || '', startedAt: null, finishedAt: null, current: 0, completed: 0, currentId: '', currentStep: '', results: new Array(ids.length), summary: null, error: null, items: ids.map(userId => ({ userId, status: 'queued', step: 'Menunggu', attempt: 0, maxAttempts: CHECK_RETRIES + 1, worker: null })) };
